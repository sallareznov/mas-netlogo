...
tunas-own [breedLimit breedCounter]
sharks-own [breedLimit breedCounter starve starveCounter]

to setup
  ca
  create-tunas nbTunas
  create-sharks nbSharks
  ask patches [set pcolor white]
  ask tunas [set breedLimit tunasBreed set breedCounter 0 setxy random-pxcor random-pycor set shape "fish" set color pink]
  ask sharks [set breedLimit sharksBreed set breedCounter 0 set starve sharksStarve set starveCounter 0 setxy random-pxcor random-pycor set shape "fish" set color blue]
  reset-ticks
end
  ...
  ifelse starveCounter = starve
    ...
    [ifelse any? neighbors with [any? tunas-here]
      [let tuna-to-be-eaten-patch one-of neighbors with [any? tunas-here]
          ...
          move-and-aim-to-reproduce tuna-to-be-eaten
      ]
      [let free-neighbor-patch one-of neighbors with [not any? turtles-here]
        ifelse (free-neighbor-patch = nobody)
        [move-and-aim-to-reproduce free-neighbor-patch]
        [set breedCounter (breedCounter + 1) set starveCounter (starveCounter + 1)]
      ]
    ]
end
